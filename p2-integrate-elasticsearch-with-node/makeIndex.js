const _ = require('highland'); //we can switch between synchronous and asynchronous data sources at will
const fs = require('fs');
const csv = require('csv-parser'); //it can convert CSV into JSON at at rate of around 90,000 rows per second
...
const start = async () => {
    ...
    let currentIndex = 0;
    const stream = _(
        fs.createReadStream('./elasticsearch/planet-latest-100k_geonames.tsv')
        .pipe(csv({
            separator: '\t',
        }))
    )
